  ...
  .handler(async ({ data }) => {
    ...
    if (signupData.user) {
      if (data.is_admin) {
        ...
        if (data.houseId) {
          if (data.houseOwner) {
            const { error: houseOwnerError } = await createHouseOwnerQuery(
              supabase,
              data.houseId,
              signupData.user.id,
            )
            if (houseOwnerError) {
              logger('error', 'Error creating house owner:', {
                error: houseOwnerError,
              })
              return {
                error: true,
          ...
          const { error: houseUserError } = await createHouseUserQuery(
            supabase,
            data.houseId,
            signupData.user.id,
          )
          if (houseUserError) {
            logger('error', 'Error creating house user:', {
              error: houseUserError,
            })
            return {
              error: true,
...
export const inviteUserFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    if (existingInvite) {
      logger('warn', 'User already invited:', {
        email: data.email,
